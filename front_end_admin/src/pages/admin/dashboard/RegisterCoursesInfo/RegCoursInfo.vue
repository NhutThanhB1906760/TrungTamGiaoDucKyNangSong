        ...
        <va-data-table :items="items" :columns="columns" :per-page="perPage" :current-page="currentPage" :filter="filter"
            ...
            <template #expandableRow="{ rowData }">
                <div class="p-4 bg-slate-300" v-if="rowData.status==1">
                    ...
                    <div v-if="getBill(rowData.id).payment_methods==0">Nhân viên thu tiền: {{ getBill(rowData.id).personnel_name }}</div>
...
const getCourses = async () => {
    items.value = []
    items.value = await RegInfoService.getAll()
...
const delCours = async (id) => {
    let item = await RegInfoService.delete(id)
    if (item) {
