<template >
    <va-card class="p-4">
        <div class="flex  flex-wrap items-center whitespace-nowrap">
            <va-button icon="add" size="large" class="mr-3 mb-2 min-w-fit" @click="addItem()">
                Thêm lớp học
            </va-button>
        ...
        <va-modal class="z-50" v-model="showModal" close-button hide-default-actions blur>
            ...
            <va-form ref="formRef" class="grid grid-cols-6 gap-4 mb-4">
                <va-input placeholder="" class="col-span-3" v-model="name"
                    :rules="[(value) => (value && value.length > 0) || 'Trường này chưa có giá trị']" label="Tên lớp học" />
                <va-input placeholder="" type="number" class="col-span-3" v-model="quantity"
                    :rules="[(value) => (value > 0) || 'Trường này chưa có giá trị']" label="Số lượng" />
                <va-date-input v-model="startTime" class="col-span-3" label="Ngày bắt đầu" required-mark=""
                    :rules="[value => value!==null || 'Trường này chưa có giá trị']" />
                <va-date-input v-model="endTime" class="col-span-3" label="Ngày kết thúc" required-mark=""
                    :rules="[value => value!==null || 'Trường này chưa có giá trị']" />
                <va-select class="col-span-2" v-model="expression" label="Hình thức"
                    :options="['Trực tiếp (offline)', 'Trực tuyến (online)']" required-mark="" />
                <va-select class="col-span-2" v-model="courses_id" label="Thuộc khóa học" :options="opCours"
                    required-mark="" :rules="[(value) => value || 'Trường này chưa có giá trị']"/>
                <va-select class="col-span-2" v-model="personnel_id" label="Giáo viên giảng dạy" :options="opTeacher"
                    required-mark="" :rules="[(value) => value || 'Trường này chưa có giá trị']"/>
                <!-- <va-select class="" v-model="rooms_id" label="Phòng học" :options="opRoom" required-mark="" /> -->
                <va-card class="mt-2 p-2 col-span-6">
                    ...
                    <div class="flex-wrap items-center">
                        <template v-for="item in studyTime" :key="item">
                            <va-checkbox v-model="selection" :array-value="item.id"
                                :label="'Thứ ' + item.thu + ': ' + formatTime(item.startTime) + '-' + formatTime(item.endTime)" class="mr-2" />
                        </template>
                    </div>
                </va-card>
                <va-button class="mx-2 col-span-6" v-if="add" @click=" submitPos()">
                    Tạo mới
                </va-button>
                <va-button class="mx-2  col-span-6" v-else @click="submitEdit()">
                    Lưu thay đổi
                </va-button>
            </va-form>
        </va-modal>
...
const columns = [
    { key: "name", label: 'Tên lớp', width: 100, sortable: true },
    { key: "quantity", label: 'Số lượng', width: 50, sortable: true },
    { key: "startTime", label: 'Ngày bắt đầu', width: 50, sortable: true },
    { key: "endTime", label: 'Ngày kết thúc', width: 50, sortable: true },
    { key: "expression", label: 'Hình thức', width: 50 },
    // { key: "room_name", label: 'Phòng học', width: 50,sortable: true },
    { key: "teacher_name", label: 'Giáo viên', width: 50, sortable: true },
    { key: "cours_name", label: 'Khóa học', width: 50, sortable: true },
...
const getData = async () => {
    ...
    classStudy.value = classStudy.value.map(itemA => {
        const matchB = rooms.value.find(itemB => itemB.id === itemA.rooms_id);
        if (matchB) {
            return { ...itemA, room_name: matchB.name };
        } else {
            return { ...itemA, room_name: null };
        }
    });

    classStudy.value = classStudy.value.map(itemA => {
        const matchB = teachers.value.find(itemB => itemB.id === itemA.personnel_id);
        if (matchB) {
            return { ...itemA, teacher_name: matchB.name };
        } else {
            return { ...itemA, teacher_name: null };
        }
    });
...
function formatForm() {
    const form = {
        name: name.value,
        startTime:  moment(startTime.value, 'DD/MM/YYYY').format('YYYY-MM-DD'),
        endTime: moment(endTime.value, 'DD/MM/YYYY').format('YYYY-MM-DD'),
