<template>
    <va-card class="p-3">
        <div class="border p-2 mb-2">
            <StarRating v-model:rating="rate" :show-rating="false" :star-size="20" />
            <va-input v-model="value" placeholder="Nội dung đánh giá..." class="mt-1"/>

        </div>
        <div class="flex justify-center">
            <va-button color="warning" class="" @click="submit">
                Đánh giá
            </va-button>
        </div>
    </va-card>
    <div>
        <va-scroll-container class="max-h-[25em]" vertical>
            <va-list>
                ...
                <va-list-item v-for="(contact, index) in data" :key="index" class="list__item">
                    ...
                    <va-list-item-section>
                        <va-list-item-label class="w-full flex items-center justify-between">
                            {{ contact?.user_name }}
                            <h6 v-if="contact.subscribers_id==subscribers_id" class="text-red-600 cursor-pointer" @click="delItem(contact.id)">Xóa</h6>
...
const subscribers_id=$cookies.get('user')?.id
onMounted(() => {
    getData()
})
